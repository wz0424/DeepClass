...
public class start1 extends BaseActivity implements SensorEventListener, View.OnClickListener {
    /**
     * 规定到达距离范围距离
     */
    ...
    public void onClick(View view) {
        if (view.getId() == R.id.arriver_bt) {
            MyHelper ggg = new MyHelper(start1.this);
            SQLiteDatabase db;
            db = ggg.getWritableDatabase();
            java.text.SimpleDateFormat df = new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
            ggg.AT(db, "attendanceT",id,df.format(new java.util.Date()),String.valueOf(mCurrentLat),String.valueOf(mCurrentLon));
            Toast.makeText(this, "开始打卡！", Toast.LENGTH_SHORT).show();
